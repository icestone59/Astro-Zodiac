...
# ------------------------------------------------------------------
# Global Exception Handler & Terminal Debug Logger
# ------------------------------------------------------------------
...
# 1. Real-time Transits Endpoint (ดาวจรปัจจุบันสดๆ)
@app.route('/transit', methods=['GET'])
def get_transit():
    transits = calculate_current_transits()
    return jsonify({"status": "success", "transits": transits})

# 2. Birth Chart Calculation Endpoint (คำนวณองศาดาวกำเนิดและเรือนชะตา)
...
# 3. AI Dispatcher (พยากรณ์พื้นดวง 7 หมวด & Transit Q&A ตอบคำถามเจาะจง)
@app.route('/analyze_ai', methods=['POST'])
def analyze_ai_endpoint():
    # สกัดข้อมูลจาก Request Payload ก่อนเรียกใช้งานเสมอ
    data = request.get_json() or {}
    ...
    # บังคับยิง API ใหม่เมื่อผู้ใช้เลือก Bypass Cache
    cached_response = None if bypass_cache else get_cached_ai_report(cache_key, report_type)
